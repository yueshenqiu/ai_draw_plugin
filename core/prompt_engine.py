...
def build_character_ref_context(ref_mode: str, char_features: Optional[Dict[str, Any]] = None) -> str:
    ...
    if char_features:
        ...
        parts.append("参考图已定义了角色的完整外貌。你必须严格只输出以下可描述内容，否则视为违规：")
        parts.append("")
        parts.append("1. 【严禁编造外貌】绝对不要编造任何发色、发型、瞳色、肤色标签，包括但不限于：")
        parts.append("   black hair, brown hair, blonde hair, white hair, red hair, pink hair, silver hair,")
        parts.append("   long hair, short hair, medium hair, twintails, ponytail, braid, bob cut, bangs,")
        parts.append("   blue eyes, green eyes, red eyes, brown eyes, purple eyes, heterochromia,")
        parts.append("   dark skin, pale skin, tan, fair skin, brown skin — 以上全部禁止")
        parts.append("")
        parts.append("2. 【严禁编造服装】除非用户明确指定了具体服装或穿搭，否则禁止输出任何服装标签，包括但不限于：")
        parts.append("   dress, skirt, shirt, blouse, jacket, sweater, hoodie, pants, shorts, jeans,")
        parts.append("   pantyhose, thighhighs, stockings, socks, barefoot, shoes, boots, heels,")
        parts.append("   underwear, bra, panties, camisole, lingerie, swimsuit, bikini,")
        parts.append("   translucent, sheer, see-through, lace, leather, denim — 以上全部禁止")
        parts.append("")
        parts.append("3. 【仅允许输出以下内容】")
        parts.append("   - 动作姿势 (pose/action/body language)")
        parts.append("   - 表情神态 (expression/emotion)")
        parts.append("   - 场景环境 (setting/background/location)")
        parts.append("   - 光影氛围 (lighting/atmosphere/time of day)")
        parts.append("   - 镜头构图 (camera angle/composition/viewpoint)")
        parts.append("")
        parts.append("4. 角色的外貌和服装完全由参考图决定。你在 prompt 中只描述用户的意图（姿势/场景/氛围），不描述角色的任何可视特征。")
