...
用户命令 → 权限/开关校验 → LLM 生成提示词（或直传 tag）
        ...
        → 经 HTTP API 直发 / 合并转发 → 延时自动撤回
```

插件**绕过 SDK 的 IPC 链路**，直接调用本地 HTTP API（NapCat 或 SnowLuma）发送和撤回消息，避免多层 RPC 超时导致的发送失败和重复发送。

> 参考图获取同时兼容 **NapCat** 和 **SnowLuma** 两种适配器：NapCat 走 `get_msg` / `get_image` 接口；SnowLuma 走 MaiBot SDK 的 `message.get_by_id(include_binary_data=True)` 获取原始图片二进制。
...
插件需要 QQ 客户端开启一个 HTTP Server 供直连发图和撤回。

**NapCat** — 在 `onebot11_<QQ号>.json` 的 `httpServers` 中新增一项：
...
配置完后把 `port` 和 `token` 填到 `config.toml` 的 `napcat_http_url` / `napcat_http_token`，再重启 QQ 客户端使端口生效。
