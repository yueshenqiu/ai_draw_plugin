...
MaiBot 的 AI 绘图插件。通过 LLM 将自然语言描述转换为结构化提示词，再调用图像生成服务出图。支持多模型热切换、风格预设（画师串）/提示词预设、参考图模式、自拍模式、自动撤回等功能。
...
OneBot HTTP 服务，超时预算更长（60 秒），绕开适配器超时。
...
| `[models.modelX]` | 图像生成模型（API、参数、风格预设/画师串） |
